document.addEventListener('DOMContentLoaded',()=>{
    ...
    document.getElementById('bookingForm').addEventListener('submit',e=>{
        ...
        const all=JSON.parse(localStorage.getItem('autofine_bookings')||'[]');all.push(d);localStorage.setItem('autofine_bookings',JSON.stringify(all));
        e.target.reset();showToast('REQUEST RECEIVED. WE WILL CONTACT YOU.');
    });
});
function bookSvc(s){document.getElementById('svcSelect').value=s;document.getElementById('booking').scrollIntoView({behavior:'smooth'})}
